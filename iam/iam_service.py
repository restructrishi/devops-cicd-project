    ...
    try:
        ...
        if instance_profile_exists(iam_client, profile_name):
            ...
            if role_attached:
                ...
                # Profile exists but no role attached - will re-attach
                print_log(f"  ⚠ Instance profile exists but has no role attached", "WARNING")
                print_log(f"   Will attempt to attach role {role_name}...")
                # Don't return - let the code proceed to attachment below
        else:
            profile_exists_flag = False
